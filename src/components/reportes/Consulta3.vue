...
export default {
    ...
    methods: {
        async generarPdf() {
            const { data } = await axios.get('http://127.0.0.1:8000/pdf3/', {responseType: 'blob', params: {'fecha1': this.fecha1 }});
        ...
        async onSubmit() {
            const { data } = await axios.post('http://127.0.0.1:8000/consulta3/', {
                "fecha1": this.fecha1,
            });
            this.data = data;
            this.bandera = true;
...
<template>
    ...
    <div v-if="bandera" class="container">
        ...
        <br>
        <h5>Cantidad de servicios en la fecha {{ this.fecha1 }} : {{ data.cantidad }}</h5>
        <hr />
        <br>
